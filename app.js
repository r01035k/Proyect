...
const supabaseKey = 'TU_SUPABASE_KEY'  // reemplaza con tu clave real
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function cargarTodosLosDocumentos(){
    ...
    if(!data || data.length===0){ weekContent.innerHTML="<p>No hay documentos registrados.</p>"; return }
    mostrarDocumentos(data)
  }

  async function cargarDocumentosPorSemana(semana){
    weekContent.innerHTML = "<p>Cargando documentos...</p>"
    const { data, error } = await supabase.from("documentos").select("*").eq("semana", semana).order("created_at",{ascending:true})
    if(error){ weekContent.innerHTML="<p>Error al cargar documentos.</p>"; console.error(error); return }
    if(!data || data.length===0){ weekContent.innerHTML=`<p>No hay documentos para la semana ${semana}.</p>`; return }
    mostrarDocumentos(data)
  }
      ...
      semanas[semana].forEach(doc=>{
        ...
        if(isAdmin){
          ...
          delBtn.onclick = async ()=>{
            ...
            if(!error){ alert("Documento eliminado"); cargarTodosLosDocumentos() } 
            else { alert("Error al eliminar"); console.error(error) }
          }
  ...
  uploadForm.addEventListener("submit", async (e)=>{
    ...
    if(error){ alert("Error al subir archivo"); console.error(error); return }
    ...
    if(insertError){ alert("Error al guardar documento"); console.error(insertError); return }
